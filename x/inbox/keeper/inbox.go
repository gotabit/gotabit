...
func (k Keeper) GetMsgById(ctx sdk.Context, id uint64) (*types.Msg, error) {
	...
	if bz == nil {
		return &types.Msg{}, fmt.Errorf("Inbox module error. Msg: %d does not exist", id)
	}
	msg := types.Msg{}
...
func (k Keeper) GetAllMsgs(ctx sdk.Context) []types.Msg {
	...
	msgs := []types.Msg{}
	it := store.Iterator(types.PrefixMsg, types.PrefixMsg)
	defer it.Close()
...
func (k Keeper) GetMsgsBySender(ctx sdk.Context, sender string) []*types.Msg {
	...
	msgs := []*types.Msg{}
	key := append(types.PrefixMsgBySender, sender...)
	it := store.Iterator(key, key)
	defer it.Close()
...
func (k Keeper) GetMsgsByReceiver(ctx sdk.Context, receiver, topics string) []*types.Msg {
	...
	it := store.Iterator(prefix, prefix)
	defer it.Close()
